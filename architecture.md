...
- CI runs but doesn't gate merges. `.github/workflows/ci.yml` runs lint for both packages, the backend unit and e2e suites against a Postgres service container, and a frontend production build, on every push and PR. `main` has no branch protection rule, so a failing run is visible on the commit but doesn't block a merge — that's the remaining gap. Pushes to `main` still trigger Vercel's and Render's git-connected deploys directly. Lint runs `eslint` directly rather than `npm run lint`, since both packages define that script with `--fix`, which would rewrite files instead of failing.
