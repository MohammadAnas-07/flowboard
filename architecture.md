...
- Confirmed: the funnel/filter icon shown next to "Fields" in the Figma toolbar has no prototyped interaction in the source file — no distinct panel or click behavior is defined for it there. It's rendered to match the design but is purely decorative: an `aria-hidden`, `focusable="false"`, `pointer-events: none` SVG, not a button, with no click handler. Inventing a second filter entry point for it would have duplicated what the "Fields" button already does (Status/Priority submenus filter rows). It lives in `FieldsDropdown` rather than in each toolbar, so all three usages stay consistent. If the Figma is ever updated with an intended behavior, wire it there.
...
- CI coverage is only as wide as the test suite behind it. `.github/workflows/ci.yml` runs lint for both packages, the backend unit and e2e suites against a Postgres service container, and a frontend production build, on every push and PR. A repository ruleset (`main-ci-gate`) targets `main` and requires the `build-and-test` check to pass, and blocks force pushes and branch deletion. That gates the pipeline, not the coverage — the frontend still has no tests, so a green run says the build compiles and lint is clean, not that the UI works. Lint runs `eslint` directly rather than `npm run lint`, since both packages define that script with `--fix`, which would rewrite files instead of failing. Pushes to `main` trigger Vercel's and Render's git-connected deploys directly.
...
None of this is built. Today the app is one Render instance talking to one Neon database, and at the traffic a demo gets that's the right size. This section is what I'd actually have to change if it needed to grow, and which existing decisions would help or get in the way.

**Auth is already stateless, which is the part that usually hurts.** The session is a signed JWT in an `httpOnly` cookie (Section 4). There's no server-side session store, no sticky sessions, and nothing to replicate between instances. Any instance holding `JWT_SECRET` can verify any request's cookie on its own, so adding instances needs no auth work at all. That was a deliberate pick, and it's the single biggest thing that makes horizontal scaling boring here.

The honest caveat: `AuthGuard` verifies the signature and then does a `prisma.user.findUnique` on every request to load the current user. So there's no *session* state on the server, but there is a database round-trip per authenticated request. That's fine now and it's the first thing I'd look at under real load. The fix is to trust the JWT's claims for the common path and only read the user row where freshness genuinely matters, or to cache the lookup behind a short TTL. Both are small changes; neither is worth making at this size.

The other cost of stateless JWTs is revocation. A token stays valid for its full 7 days, so `POST /api/auth/logout` clearing the cookie is a client-side gesture, not a server-side kill. If a captured token ever mattered, the answer is short-lived access tokens plus refresh, or a revocation denylist, and a denylist puts shared state back into a design that currently doesn't need any.

**Running more than one backend instance.** Render's free tier is single-instance, so this can't be tested today. If it were a paid plan with a higher instance count, most of the app moves over untouched. The one thing that would be quietly wrong is rate limiting. `ThrottlerModule` (Section 2, added in the security pass) uses the default in-memory storage, which means each instance keeps its own counter. With N instances the effective ceiling becomes roughly N times the configured limit, and which counter a caller lands on depends on how requests get routed. It wouldn't fail loudly, it would just stop enforcing what it claims to, which is worse. The fix is `@nestjs/throttler-storage-redis` against a shared Redis so all instances count into one bucket.

Worth flagging alongside that: per-IP limiting already doesn't identify end users, because `next.config.ts` rewrites `/api/*` through Vercel so legitimate traffic reaches the backend from a Vercel egress address. The limit is set to 100 per 15 minutes partly to absorb that. Moving to shared storage would make the counting correct across instances, but it wouldn't make the attribution correct. That needs the throttler's `getTracker()` overridden to read the forwarded client address, done carefully enough that a direct caller can't spoof past it.

**Database.** `DATABASE_URL` already points at Neon's pooled endpoint, so connections go through PgBouncer rather than straight at Postgres. Short-lived application connections multiplex onto a much smaller number of real backend connections, which is why a single instance never comes close to the ceiling and why adding a few more wouldn't either. The exact concurrent-connection cap depends on the Neon plan.

What would need attention is Prisma's own pool. No `connection_limit` is set on the URL, so Prisma's default applies per instance, and that default is derived from the CPU count of whatever machine it lands on. One instance, no problem. Several instances each opening a default-sized pool multiplies into the pooler, so I'd pin `connection_limit` explicitly rather than let it be inferred. Beyond that, the query shape would bite before the connection count does: `GET /api/tasks` and `GET /api/projects` return everything with no pagination (Section 9), so those are the first things to fix, ahead of anything infrastructural. Read replicas only start making sense well past that point.

**Load balancing.** There isn't a load balancer to discuss, because there's nothing to balance. Render terminates TLS and routes to the single instance, and `render.yaml` already points its health check at `/health`. If this ever ran multi-instance, Render's own routing would sit in front and the app wouldn't need anything custom, precisely because the auth design above requires no session affinity. Two things would need checking at that point rather than assumed: graceful shutdown, so in-flight requests finish when an instance is rotated out, and the `trust proxy` hop count in `main.ts`, which is currently set to 1 for exactly one proxy in front. Adding another layer without updating it would break client IP attribution and, with it, the rate limiter.
