...
- CI covers tests and build, but not lint, and doesn't gate merges. `.github/workflows/ci.yml` runs the backend unit and e2e suites against a Postgres service container plus a frontend production build, on every push and PR. Two gaps remain: frontend lint is left out because it currently reports four `react-hooks/purity` errors (backend lint is clean), and `main` has no branch protection rule, so a failing run is visible but doesn't block a merge. Pushes to `main` still trigger Vercel's and Render's git-connected deploys directly.
